...
func createTestPeer(t *testing.T) *Peer {
	peer, err := NewPeer("localhost", TEST_DB)
	assert.Equal(t, err, nil)
	return peer
}

func destroyTestPeer(peer *Peer) {
	peer.PrefixTree.(*prefixTree).settings.session.DB(TEST_DB).DropDatabase()
	peer.PrefixTree.(*prefixTree).settings.session.Close()
}
