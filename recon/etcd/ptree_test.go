...
func startEtcd(t *testing.T) {
	stopEtcd()
	etcdDir, _ := ioutil.TempDir("", "etcd_test")
	etcdCmd := exec.Command("etcd",
		fmt.Sprintf("-name=conflux_etcd_test"),
		fmt.Sprintf("-data-dir=%s", etcdDir),
	)
	err := etcdCmd.Start()
	if err != nil {
		t.Fatalf("%v", err)
	...
	for i := 0; i < 5; i++ {
		_, err := http.Get("http://127.0.0.1:4001/v2/stats/store")
		if err == nil {
			return
		}
		time.Sleep(time.Second)
	}
	t.Fatalf("Failed to connect to etcd")
}

func createTestPeer(t *testing.T) *recon.Peer {
	startEtcd(t)
	settings := DefaultSettings()
	settings.Set("conflux.recon.etcd.machines", "127.0.0.1:4001")
