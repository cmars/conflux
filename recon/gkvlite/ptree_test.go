...
func createTestPeer(t *testing.T) *recon.Peer {
	...
	tmpfile, err := ioutil.TempFile("", "gkvlite_test")
	assert.Equal(t, err, nil)
	settings.Set("conflux.recon.gkvlite.path", tmpfile)
...
func destroyTestPeer(peer *recon.Peer) {
	//os.RemoveAll(peer.Settings.GetString("conflux.recon.gkvlite.path", ""))
}
